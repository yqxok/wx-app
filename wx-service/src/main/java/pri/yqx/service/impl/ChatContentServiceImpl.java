...
//    }
    ...
    public ChatContentVo saveAndGetChatContentVo(ChatContentDto chatContentDto) {
        ...
        if(Objects.equals(chatContentDto.getReceiveUserId(), chatContentDto.getSendUserId()))
            throw new BusinessException("receiveUserId不能等于sendUserId");
        this.userService.validateUserId(chatContentDto.getReceiveUserId());
        this.userService.validateUserId(chatContentDto.getSendUserId());
    ...
    public void validateChatId(Long chatId) {
        ...
        if(count<1)
            throw new BusinessException("该chatId不存在");
    }
